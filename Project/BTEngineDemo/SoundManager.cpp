...
void SoundManager::PlayWAV1(int TrackNum)
{
	// 반복재생
	ynSndPlay(pSoundEffect[TrackNum], PLAY_LOOP_);
}

void SoundManager::PlayWAV2(int TrackNum)
{
	// 한 번만 재생
	ynSndPlay(pSoundEffect[TrackNum], PLAY_ONCE_);
}
...
void SoundManager::Volume()
{
	...
	//------------------------------------
	if (IsKeyDn(VK_Q)) { ynSndMasterVolumeUp(100); }
	if (IsKeyDn(VK_W)) { ynSndMasterVolumeDn(100); }
}
